...
class SQLHandler:
    ...
    def resetdb(self):
        try:
            ...
            self.execute(DROP_REVIEWSTABLE)
        except:
            "First setup of tables!"
        self.execute(CREATE_REVIEWSTABLE)
    ...
    def insert_into_videoinfo(self,name,address):
        try:
            self.execute('INSERT INTO videoinfo (videoname , address) values(?,?) ',(name,address))
            self.handle.commit()
        except:
    ...
    def insert_into_imdbtable(self,dic):
        self.execute('INSERT INTO imdbtable  values(?,?,?,?,?)',(dic['imdb_id'],dic['url'],dic['rating'],dic['votes'],dic['imdb_top250']))
        self.handle.commit()
    ...
    def insert_into_review_table(self,dic):
        imdb_id = dic["imdb_id"]
    ...
    def delete_record(self,id):
        ...
        try:
            ...
            self.handle.execute("DELETE FROM REVIEWS_TABLE WHERE imdb_id == %d" % imdb_id)
